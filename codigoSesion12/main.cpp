...
int objective(instance* I,int* solucion,int n) { //evaluar
  int obj=0;
  int t[I->m];
...
void insertar(int* origen,int* destino,int id,int lengthOrigen,int posicion) { //insertar el job id en la secuencia en la posición, posicion
  for(int i=0;i<posicion;i++) destino[i]=origen[i]; //color rojo en las láminas
  destino[posicion]=id; //color naranja en las láminas
  for(int i=posicion;i<lengthOrigen;i++) destino[i+1]=origen[i]; //color rosa en las láminas
}

//elegir entre las tareas ardías la que tiene mayor w,
//en caso que ninguna tardía escoger la de menor d
void neh(instance* I) { //lógica completa de NEH
  int solucion[I->n];
  int solucionTest[I->n];
  int solucionBest[I->n];
  paraOrden orden[I->n];

  //sumar las duraciones de cada job y lo guarda en peso
  for(int i=0;i<I->n;i++) {
    orden[i].id=i;
    orden[i].peso=0;
  ...
  //https://stackoverflow.com/questions/8721189/how-to-sort-an-array-of-structs-in-c
  //orden indicará los jobs por peso decreciente (orden[0].id tiene mayor peso que orden[1].id ,...)
  ...
  for(int i=2;i<I->n;i++) { //insertar cada pieza
    //testear todas las secuencias y quedarse con la mejor
    int best=RAND_MAX;
    for(int pos=0;pos<=i;pos++) { //ver cuál es la mejor posición
  ...
  } //se repite hasta que ya he construido la secuencia completa
  printf("solucion: ");
